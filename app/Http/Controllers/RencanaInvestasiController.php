    ...
    public function index(Request $request)
{
    ...
    $query = RencanaInvestasi::with([
        ...
        'periods' => function ($q) use ($tahun, $bulan, $week, $request) {
            $q->where('year', $tahun);
            $q->where('month', $request->filled('bulan') ? (int)$request->bulan : $bulan);
            $q->where('week',  $request->filled('week')  ? (int)$request->week  : $week);
        },
    ])
    ...
    ->when($request->filled('erkap_id'), fn($q) => $q->where('erkap_id', (int)$request->erkap_id)) 
